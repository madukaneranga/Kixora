...
-- =====================================================
-- ADMIN POLICIES (FULL CATALOG MANAGEMENT)
-- =====================================================

-- Admin users can manage catalog data
CREATE POLICY "admin_catalog_access" ON categories
  FOR ALL TO authenticated
  USING (
...
CREATE POLICY "admin_variants_access" ON product_variants
  FOR ALL TO authenticated
  USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
  )
...
CREATE POLICY "admin_images_access" ON product_images
  FOR ALL TO authenticated
  USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
  )
...
CREATE POLICY "admin_collections_access" ON collections
  FOR ALL TO authenticated
  USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
  )
...
CREATE POLICY "admin_collection_products_access" ON collection_products
  FOR ALL TO authenticated
  USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
  )
...
-- Admin can access all reviews for moderation
CREATE POLICY "admin_reviews_access" ON reviews
  FOR ALL TO authenticated
  USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
  )
...
-- Admin can access all order items
CREATE POLICY "admin_order_items_access" ON order_items
  FOR ALL TO authenticated
  USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
  )
